    ...
    <div className="w-full overflow-hidden">
      <div className={`flex ${directionClass} gap-12`}>
        ...
        {[...items, ...items].map((item, index) => (
          <motion.div
            key={index}
            className="flex-none w-[300px] p-6 bg-customBgColor-bg border border-x-4 border-customBorderColor rounded-xl shadow-md dark:bg-green-800 dark:text-white"
          ...
          >
            ...
            <p className="mt-4 text-sm text-customTextColor-light dark:text-gray-400">– {item.name}, <span className="italic">{item.title}</span></p>
          </motion.div>
        ))}
      </div>
